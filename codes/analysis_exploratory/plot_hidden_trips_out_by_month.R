...
in_file$upper_bound = as.character(in_file$upper_bound)

pdf("Hidden_Tripsi_Out.pdf", 100, 5)

for(s_name in levels(in_file$from_station)){
	subset = in_file[in_file$from_station == s_name, ]
	print(dim(subset))

	subset_date = subset[(subset$upper_bound > "2013-04-30") & (subset$upper_bound < "2013-06-01"),]
	print(dim(subset_date))

	if(nrow(subset_date) > 0){
		print(nrow(subset_date))
		print(s_name)
		plot(0,0, type = "n", xlim = range(subset_date$upper_bound_numeric), ylim = c(1,10), main = s_name)
		...
		pain = (1:100)*86400 +(floor(min(subset_date$upper_bound_numeric) / 86400) * 86400)
